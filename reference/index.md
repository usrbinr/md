# Package index

## All functions

- [`alter_table_schema()`](https://usrbinr.github.io/md/reference/alter_table_schema.md)
  : Move Tables from One Schema to Another
- [`cd()`](https://usrbinr.github.io/md/reference/cd.md) : Change Active
  Database and Schema
- [`config_csv`](https://usrbinr.github.io/md/reference/config_csv.md) :
  DuckDB CSV read configuration (config_csv)
- [`config_db`](https://usrbinr.github.io/md/reference/config_db.md) :
  DuckDB runtime database configuration (config_db)
- [`config_excel`](https://usrbinr.github.io/md/reference/config_excel.md)
  : DuckDB Excel read configuration (config_excel)
- [`config_parquet`](https://usrbinr.github.io/md/reference/config_parquet.md)
  : DuckDB Parquet read configuration (config_parquet)
- [`configure_md_user_settings()`](https://usrbinr.github.io/md/reference/configure_md_user_settings.md)
  : Configure a MotherDuck user's settings
- [`connect_to_motherduck()`](https://usrbinr.github.io/md/reference/connect_to_motherduck.md)
  : Create connection to motherduck
- [`copy_tables_to_new_location()`](https://usrbinr.github.io/md/reference/copy_tables_to_new_location.md)
  : Copy Tables to a New Database/Schema
- [`create_database()`](https://usrbinr.github.io/md/reference/create_database.md)
  : Create (If Not Exists) and Switch to a Database
- [`create_if_not_exists_share()`](https://usrbinr.github.io/md/reference/create_if_not_exists_share.md)
  : Create a MotherDuck database share if it does not exist
- [`create_md_access_token()`](https://usrbinr.github.io/md/reference/create_md_access_token.md)
  : Create a MotherDuck access token
- [`create_md_user()`](https://usrbinr.github.io/md/reference/create_md_user.md)
  : Create a new MotherDuck user
- [`create_or_replace_share()`](https://usrbinr.github.io/md/reference/create_or_replace_share.md)
  : Create or replace a MotherDuck database share
- [`create_schema()`](https://usrbinr.github.io/md/reference/create_schema.md)
  : Create a Schema in a Database if It Does Not Exist
- [`create_table()`](https://usrbinr.github.io/md/reference/create_table.md)
  : Create or Append a Table from a Tibble or DBI-Backed Table
- [`delete_and_create_schema()`](https://usrbinr.github.io/md/reference/delete_and_create_schema.md)
  : Drop and Recreate a Schema in a MotherDuck / DuckDB Database
- [`delete_database()`](https://usrbinr.github.io/md/reference/delete_database.md)
  : Drop a Database
- [`delete_md_access_token()`](https://usrbinr.github.io/md/reference/delete_md_access_token.md)
  : Delete a MotherDuck user's access token
- [`delete_md_user()`](https://usrbinr.github.io/md/reference/delete_md_user.md)
  : Delete a MotherDuck user
- [`delete_schema()`](https://usrbinr.github.io/md/reference/delete_schema.md)
  : Drop a Schema from a Database
- [`delete_table()`](https://usrbinr.github.io/md/reference/delete_table.md)
  : Drop a Table
- [`describe_share()`](https://usrbinr.github.io/md/reference/describe_share.md)
  : Describe a MotherDuck share
- [`drop_share()`](https://usrbinr.github.io/md/reference/drop_share.md)
  : Drop a MotherDuck share
- [`install_extensions()`](https://usrbinr.github.io/md/reference/install_extensions.md)
  : Install DuckDB/MotherDuck Extensions
- [`launch_ui()`](https://usrbinr.github.io/md/reference/launch_ui.md) :
  Launch the DuckDB UI in your browser
- [`list_all_databases()`](https://usrbinr.github.io/md/reference/list_all_databases.md)
  : List Databases Visible to the Connection
- [`list_all_tables()`](https://usrbinr.github.io/md/reference/list_all_tables.md)
  : List All Tables Visible to the Connection
- [`list_current_schemas()`](https://usrbinr.github.io/md/reference/list_current_schemas.md)
  : List Schemas in the Current Database
- [`list_current_tables()`](https://usrbinr.github.io/md/reference/list_current_tables.md)
  : List Tables in the Current Database and Schema
- [`list_extensions()`](https://usrbinr.github.io/md/reference/list_extensions.md)
  : List MotherDuck/DuckDB Extensions
- [`list_fns()`](https://usrbinr.github.io/md/reference/list_fns.md) :
  List Database Functions (DuckDB/MotherDuck)
- [`list_md_active_accounts()`](https://usrbinr.github.io/md/reference/list_md_active_accounts.md)
  : List active MotherDuck accounts
- [`list_md_user_instance()`](https://usrbinr.github.io/md/reference/list_md_user_instance.md)
  : List a MotherDuck user's instance settings
- [`list_md_user_tokens()`](https://usrbinr.github.io/md/reference/list_md_user_tokens.md)
  : List a MotherDuck user's tokens
- [`list_owned_shares()`](https://usrbinr.github.io/md/reference/list_owned_shares.md)
  : List all shares owned by the current user
- [`list_setting()`](https://usrbinr.github.io/md/reference/list_setting.md)
  : List Database Settings
- [`list_shared_with_me_shares()`](https://usrbinr.github.io/md/reference/list_shared_with_me_shares.md)
  : List all shares shared with the current user
- [`list_shares()`](https://usrbinr.github.io/md/reference/list_shares.md)
  : List MotherDuck Shares
- [`load_extensions()`](https://usrbinr.github.io/md/reference/load_extensions.md)
  : Load and Install DuckDB/MotherDuck Extensions
- [`pwd()`](https://usrbinr.github.io/md/reference/pwd.md) : Print
  Current MotherDuck Database Context
- [`read_csv()`](https://usrbinr.github.io/md/reference/read_csv.md) :
  Read a CSV file into a DuckDB/MotherDuck table
- [`read_excel()`](https://usrbinr.github.io/md/reference/read_excel.md)
  : Read an Excel file into a DuckDB/MotherDuck table
- [`show_current_user()`](https://usrbinr.github.io/md/reference/show_current_user.md)
  : Show current database user
- [`show_motherduck_token()`](https://usrbinr.github.io/md/reference/show_motherduck_token.md)
  : Show Your MotherDuck Token
- [`summary(`*`<tbl_lazy>`*`)`](https://usrbinr.github.io/md/reference/summary.md)
  : Summarize a Lazy DBI Table
- [`upload_database_to_md()`](https://usrbinr.github.io/md/reference/upload_database_to_md.md)
  : Upload a Local Database to MotherDuck
- [`validate_and_print_database_loction()`](https://usrbinr.github.io/md/reference/validate_and_print_database_loction.md)
  : validate and Pprint your database location
- [`validate_con()`](https://usrbinr.github.io/md/reference/validate_con.md)
  : Validate connection is DuckDB
- [`validate_extension_install_status()`](https://usrbinr.github.io/md/reference/validate_extension_install_status.md)
  : Validate Installed MotherDuck/DuckDB Extensions
- [`validate_extension_load_status()`](https://usrbinr.github.io/md/reference/validate_extension_load_status.md)
  : Validate Loaded MotherDuck/DuckDB Extensions
- [`validate_md_connection_status()`](https://usrbinr.github.io/md/reference/validate_md_connection_status.md)
  : Validate Mother Duck Connection Status
